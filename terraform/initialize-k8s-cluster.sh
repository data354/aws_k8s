...
master_ansible_pub_key=$(ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa "cat ~/.ssh/id_rsa.pub")
...
check_file_control_plane_1=$(ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa "cat /etc/hosts | grep control-plane-1")
check_file_data_plane_1=$(ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa "cat /etc/hosts | grep data-plane-2")
check_file_data_plane_2=$(ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa "cat /etc/hosts | grep data-plane-2")
check_file_data_plane_3=$(ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa "cat /etc/hosts | grep data-plane-3")

# Ajouter les hostname des machines dans le master ansible

if [ -z "$check_file_control_plane_1" ]; then
  ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa 'sudo bash -c "echo \"10.240.0.5 control-plane-1\" >> /etc/hosts"' &
fi

if [ -z "$check_file_data_plane_1" ]; then
  ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa 'sudo bash -c "echo \"10.240.0.6 data-plane-1\" >> /etc/hosts"' &
fi

if [ -z "$check_file_data_plane_2" ]; then
  ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa 'sudo bash -c "echo \"10.240.0.7 data-plane-2\" >> /etc/hosts"' &
fi

if [ -z "$check_file_data_plane_3" ]; then
  ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa 'sudo bash -c "echo \"10.240.0.8 data-plane-3\" >> /etc/hosts"' &
fi
...
ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa "ssh-keyscan -H github.com >> ~/.ssh/known_hosts"
...
echo "Connection to Master ansible ... $user@$master_ansible_public_ip"
ssh $user@$master_ansible_public_ip -i ~/.ssh/id_rsa 
